...
function adminOnly(req, res, next) {
  ...
  try {
    ...
    if (decoded.email !== "tiegmicheal@gmail.com" || decoded.role !== "admin") {
...
app.post("/ai/create-store", auth, async (req, res) => {
  try {
    const prompt = `
Create ecommerce store for ${req.body.niche}.
Return JSON: {products:[{name,price}], description}
`;
    ...
    res.json({ store, products, description: data.description });
...
// ================= AI WEBSITE BUILDER =================
app.post("/ai/create-website", auth, async (req, res) => {
  try {
    const prompt = `
Create a full modern responsive website for:
"${req.body.prompt}"
